...
public class AclUserPermissionSpecification implements AclSpecification {
	...
	private boolean hasRolePermission(From<?, ?> from, String permission) {
		...
		for (RolePermissionData rolePermissionData : metaData.getRolePermissionList()) {
			if (aclUtils.hasAnyAuthorities(rolePermissionData.getAuthorities()) && rolePermissionData.hasPermission(permission)) {
				return true;
			}
		}
		return false;
	...
	private boolean hasRoleCondition(From<?, ?> from, String permission) {
		...
		for (RolePermissionData roleConditionData : metaData.getRoleConditionList()) {
			if (aclUtils.hasAnyAuthorities(roleConditionData.getAuthorities()) && roleConditionData.hasPermission(permission)) {
				return true;
			}
		}
		return false;
	 ...
	 * @param permission
	 */
	private List<Predicate> createOwnerPredicates(From<?, ?> from, CriteriaBuilder cb, Serializable userId, String permission, boolean ownerGroup) {
		List<Predicate> predicates = new ArrayList<>();
		AclEntityMetaData metaData = aclMetaData.getAclEntityMetaData(from.getJavaType());
	...
	private List<Predicate> createOwnerGroupPredicates(From<?, ?> from, CriteriaBuilder cb, Serializable userId, String permission) {
		...
		for (OwnerData ownerGroupData : metaData.getOwnerGroupDataList()) {
			if (ownerGroupData.hasPermission(permission)) {
				...
				predicates.addAll(createOwnerPredicates(from.join(ownerGroupData.getPropertyName(), JoinType.LEFT), cb, userId, permission, true));
			}
		}
		return predicates;
	}
